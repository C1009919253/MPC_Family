...
int main( )
{
    ...
    f << next(x) == x + cos(theta)*v*dt; // BUG!!!
    f << next(y) == y + sin(theta)*v*dt;
    f << next(theta) == theta + w*dt;
...
int main( )
{
    ...
    // Variables:
    DifferentialState xe;
    DifferentialState ye;
    DifferentialState thetae;
    Control v;
    Control w;
